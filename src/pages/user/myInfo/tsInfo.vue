  ...
  methods: {
    ...
    updatePersonInfo(person_info) {
      this.$refs[person_info].validate(valid => {
        if (valid) {
          let URL = `${apiPath}/teacher/changeStTeacherPersonal`;
          ...
          axios({
            ...
            .then(res => {
              if (res.data.code === 200) {
                ...
                localStorage.setItem("email",this.person_info.email)
    ...
    add() {
      ...
      if (!(this.theCourse.startTime == "" || this.theCourse.endTime == "")) {
        ...
        var year2 = this.theCourse.startTime.substring(0, 4);
        var month2 = this.theCourse.startTime.substring(5, 7) - 1;
        var day2 = this.theCourse.startTime.substring(8, 10);
        var hours2 = this.theCourse.startTime.substring(11, 13);
        var minutes2 = this.theCourse.startTime.substring(14, 16);
        var seconds2 = this.theCourse.startTime.substring(17, 19);
        let time2 = new Date(year2, month2, day2, hours2, minutes2, seconds2);
        if (!(time2.getTime() > newDate.getTime())) {
          this.$Message.warning({
            content: `设置的时间距离当前不足24小时`
          });
          return;
        }
if(this.theCourse.startTime == this.theCourse.endTime){
           this.$Message.warning({
            content: `开始时间和结束时间不能相同`
          });
          return;
        }
    ...
    loadData() {
      let teacherid = localStorage.getItem("userID");
      let URL = `${apiPath}/teacher/showPersonalInformation?userId=${teacherid}`;
      axios({
        url: URL,
        method: "get"
      })
    ...
    changeimg(src) {
      let teacherid = localStorage.getItem("userID");
      let URL = `${apiPath}/teacher/changeStteacherImage`;
    ...
    handleBeforeUpload(file) {
      ...
      axios
        ...
        .then(res => {
          // this.person_info.imagesrc = "https://202.120.117.43/" + res.data;
          this.person_info.imagesrc = "https://writingcenter.shu.edu.cn/" + res.data;
